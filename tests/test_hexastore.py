...
class test_create(TestCase):
    ...
    def test_addDictAsPath(self):
        ...
        self.db.addDictAsPath({'know': 'joe',
                               'lol': {'wow': 'hab'},
                               'a': 1}, 'bob')
    ...
    def test_addDictAsJSON(self):
        added = self.db.addDictAsJSON({'know': 'joe',
                                       'lol': {'wow': 'hab'},
                                       'a': 1})
        self.assertEqual(self.db.size(), 4)
    ...
    def test_addDictAsUUID(self):
        added = self.db.addDictAsUUID({'know': 'joe',
                                       'lol': {'wow': 'hab'},
                                       'a': 1})
        self.assertEqual(self.db.size(), 4)
        self.assertIn([added, 'know', 'joe', True], self.db.all())
...
class test_search(TestCase):
    ...
    def test_search_filter(self):        
        res = self.db.search([
            ["<bizer/bsbm/v01/instances/ProductType1>", ["predicate"], ["object"]],
    ...
    def test_precise_search(self):
        ...
        self.assertEqual('<bizer/bsbm/v01/instances/StandardizationInstitution1>',
                         res[0]['publisher'])
